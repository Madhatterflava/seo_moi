...
var LanderJS = (function() {
    ...
    LANDER.check = function() {
        if (typeof jQuery == 'undefined') {
            LANDER.loadJS('https://cdn.synergy.ru/libs/jquery/jquery-1.12.4.min.js', LANDER.check);
            return;
        }
    ...
    LANDER.initAutoSubmit = function() {
        ...
        var uris = [
            ...
            'synergymba.ru',
            'synergy.ru',
            'sbs.edu.ru/lp/belochkina/kou-v2',
    ...
    if ((window.location.hostname.indexOf('synergyglobal.ru') != -1) && LANDER.getQuery('partner') !== false) {
        ...
        document.cookie = 'GlobalPartner=;path=/';
    }
    ...
    if (
        loc.indexOf('internet.synergy.ru') != -1 
        ||
        loc.indexOf('synergy.ru/bs/smartmba') != -1
        ||
        loc.indexOf('upw360.ru') != -1
        ) {
            if (
                LANDER.getQuery('admitad_uid') !== false
                &&
                LANDER.getQuery('admitad_uid') != ''
                && 
                LANDER.get_cookie('adviero_uid') == null
                &&
                LANDER.get_cookie('advcake_trackid') == null
                &&
                LANDER.get_cookie('admitad_uid') == null
                ) {
                var admitad_uid = LANDER.getQuery('admitad_uid');
                LANDER.set_cookie('admitad_uid', admitad_uid, {
                    expires: Date.now() + 60 * 60 * 24 * 30 * 1000
                });
            } else if(LANDER.get_cookie('admitad_uid') != null){
                var admitad_uid = LANDER.get_cookie('admitad_uid');
            }        
        }

    if (
        LANDER.getQuery('utm_source') == 'adviero' 
        && 
        LANDER.get_cookie('adviero_uid') == null
        &&
        LANDER.get_cookie('advcake_trackid') == null
        ...
        var adviero_uid = LANDER.getQuery('click_id');
        LANDER.set_cookie('adviero_uid', adviero_uid, {
            expires: Date.now() + 60 * 60 * 24 * 30 * 1000
        });
    } else if(LANDER.get_cookie('adviero_uid') != null){
        var adviero_uid = LANDER.get_cookie('adviero_uid');
    }

    if (
        LANDER.getQuery('utm_source') == 'advcake' 
        && 
        LANDER.get_cookie('adviero_uid') == null
        &&
        LANDER.get_cookie('advcake_trackid') == null
        ...
        var advcake_trackid = [...Array(32)].map(i=>(~~(Math.random()*36)).toString(36)).join('');
        LANDER.set_cookie('advcake_trackid', advcake_trackid, {
            expires: Date.now() + 60 * 60 * 24 * 30 * 1000
        });
    } else if(LANDER.get_cookie('advcake_trackid') != null){
        var advcake_trackid = LANDER.get_cookie('advcake_trackid');
    }

    if (
        LANDER.getQuery('utm_source') == 'salid' 
        && 
        LANDER.get_cookie('adviero_uid') == null
        &&
        LANDER.get_cookie('advcake_trackid') == null
        ...
        var salid_uid = LANDER.getQuery('utm_term');
    }

    if (
        LANDER.getQuery('utm_source') == 'cityads' 
        && 
        LANDER.get_cookie('adviero_uid') == null
        &&
        LANDER.get_cookie('advcake_trackid') == null
        ...
        var cityads_uid = LANDER.getQuery('click_id');
    }

    if(LANDER.get_cookie('entry_point') != null){
        var entry_point = LANDER.get_cookie('entry_point');
    } else {
        var entry_point = loc;
        LANDER.set_cookie('entry_point', entry_point, {
            expires: Date.now() + 60 * 60 * 24 * 30 * 1000
        });
    }

    //Для synergy.ru и всех его лендов не включать куки!
    if (!skipCookie &&
            (
                ...
                (window.location.hostname.indexOf('moi.edu.ru') != -1) ||
                (window.location.hostname.indexOf('synergyregions.ru') != -1)
            )) {
        if (window.location.search.indexOf('utm_') != -1) {
            ...
            for (var i = 0, len = utmArr.length; i < len; i++) {
                ...
                if (currUtm !== false && currUtm != '' && LANDER.get_cookie(utmArr[i]) == null) {
                    ...
                    document.cookie = utmArr[i] + '=' + currUtm + datecookie /*+ ';path=/'*/;//установить куку, если она еще не была установлена
                }
            }
            if (
            (window.location.hostname.indexOf('synergygenesis.ru') != -1) ||
    ...
    LANDER.form = function($form_obj) {
        if (typeof $form_obj == 'undefined') {
            $form_obj = $('form:not(.nolander)');
        }
        ...
        $form_obj.on('submit', function() {
                ...
                var
                    ...
                    mergelead = sessionStorage.getItem('mergelead');
                ...
                if (    window.location.hostname.indexOf('synergyonline.ru') != -1 ||
                        ...
                        window.location.hostname.indexOf('sbs.edu.ru') != -1 ||
                        window.location.hostname.indexOf('mosap.ru') != -1 ||
                        window.location.hostname.indexOf('xn--80aayoegldhg0a2a2j.xn--p1ai') != -1 ||
                        window.location.hostname.indexOf('moi.edu.ru') != -1 ||
                        window.location.hostname.indexOf('synergyregions.ru') != -1
                       ) {
                ...
                if (!mergelead && !$form.find('[name=mergelead]').length) {
                    sessionStorage.setItem('mergelead', guid + UNIX_TIMESTAMP);
                    mergelead = sessionStorage.getItem('mergelead');
                    $form.append('<input type="hidden" name="mergelead" value="' + mergelead + '">');
                } else if ($form.find('[name=mergelead]').length) {
                    mergelead = $form.find('[name=mergelead]').val();
                } else {
                    $form.append('<input type="hidden" name="mergelead" value="' + mergelead + '">');
                }
                ...
                var ubtcuid = LANDER.get_cookie('_ubtcuid');
                if (ubtcuid !== undefined && ubtcuid !== null) {
                    if (!$form.find('[name=ubtcuid]').length) {
                        $form.append('<input type="hidden" name="ubtcuid" value="' + ubtcuid + '">');
                    } else {
                        $form.find('[name=ubtcuid]').val(ubtcuid);
                    }
                }

                if (adviero_uid) {
                    $form.append('<input type="hidden" name="adv_source" value="adviero">');
                    $form.append('<input type="hidden" name="adv_uid" value="' + adviero_uid + '">');
                } else if (admitad_uid) {
                    $form.append('<input type="hidden" name="adv_source" value="admitad">');
                    $form.append('<input type="hidden" name="adv_uid" value="' + admitad_uid + '">');
                } else if (advcake_trackid) {
                    $form.append('<input type="hidden" name="adv_source" value="advcake">');
                    $form.append('<input type="hidden" name="adv_uid" value="' + advcake_trackid + '">');
                } else if (salid_uid) {
                    $form.append('<input type="hidden" name="adv_source" value="salid">');
                    $form.append('<input type="hidden" name="adv_uid" value="' + salid_uid + '">');
                } else if (cityads_uid) {
                    $form.append('<input type="hidden" name="adv_source" value="cityads">');
                    $form.append('<input type="hidden" name="adv_uid" value="' + cityads_uid + '">');
                }

                /* Проброс значения matomo_uid в лендер */
                LANDER.initMatomo(this_form);

                //Служебная информация
                $form.append('<input type="hidden" name="url_location" value="' + window.location.toString() + '">');

                if (entry_point) {
                    $form.append('<input type="hidden" name="entry_point" value="' + entry_point + '">');
                }
            ...
            .on('click', function(e) {
                ...
                if (e.ctrlKey || e.metaKey) {
                    ...
                    var inputCategoryName =  $(e.target).parents('form').find('input[name=category]').val();
                    var inputCategoryId =  $(e.target).parents('form').find('input[name=category_id]').val();
                    var inputEvent =  $(e.target).parents('form').find('input[name=event_id]').val();
                    var callbackSuccess =  $(e.target).parents('form').find('input[name=callbackSuccess]').val();

                    var tplForm = '<div id="lander-form-test-payments" style="z-index: 99991; display: table; width: 100%; height: 100%; position: fixed; left:0; top:0"><style>#lander-form-test-payments-form .lander-form__group{margin-bottom:10px}#lander-form-test-payments-form .lander-form__label{text-align:left;display:block;margin-bottom:0;font-size:10px}#lander-form-test-payments-form .lander-form__input{color:#000 !important;padding:10px;outline:none;border-radius:4px;border:1px solid #999;width:100%;max-width:300px;box-sizing:border-box}#lander-form-test-payments-form .lander-form__button{padding:15px;outline:none;border:0;border-radius:4px;background-color:#4CAF50;color:#fff;width:100%;max-width:300px;box-sizing:border-box}#lander-form-test-payments-form-submit iframe{border:0}</style><div style="display: table-cell; vertical-align: middle; text-align: center;font-family: Calibri!important;"><div style="background: rgb(255, 255, 255); min-width: 500px; padding: 10px 20px 20px; border-radius: 5px; text-align: center; display: inline-block; margin: 0 auto; box-shadow: 0 0 25px rgba(0, 0, 0, 0.5); width: 900px;"><h2 style="color: #575757;font-size: 30px;text-align: center;font-weight: 600;text-transform: none;position: relative;margin: 10px 0 15px;padding: 0;line-height: 40px;display: block;">Тестирование оплаты</h2><table width="100%" cellspacing="0" cellpadding="0" border="0"><tr><td width="50%" valign="top"><form action="' + currentAction + '" id="lander-form-test-payments-form"><div class="lander-form__inner"><div class="lander-form__group"> <input name="name" type="text" placeholder="name" class="lander-form__input" required=""></div><div class="lander-form__group"> <input name="phone" type="text" placeholder="phone" class="lander-form__input" required=""></div><div class="lander-form__group"> <input name="email" type="text" placeholder="email" class="lander-form__input" required=""></div><div class="lander-form__group"> <input name="tickets_count" type="number" placeholder="Количество билетов" value="" class="lander-form__input" min="1" required=""></div><div class="lander-form__group"> <label class="lander-form__label" for="promocode">promocode</label> <input name="promocode" type="text" placeholder="promocode" class="lander-form__input"></div><div class="lander-form__group"> <label class="lander-form__label" for="product_id">product_id (в формате 45989850)</label> <input id="lander-form-productid" name="product_id" type="text" placeholder="product_id" class="lander-form__input" required=""></div><div class="lander-form__group"><h3>Вспомогательные поля для билетных мероприятий</h3></div><div class="lander-form__group"> <label class="lander-form__label" for="category">category (наименование)</label> <input id="lander-form-categoryid" name="category" type="text" placeholder="category" class="lander-form__input" value="' + inputCategoryName + '"></div><div class="lander-form__group"> <label class="lander-form__label" for="event_id">category_id (число)</label> <input id="lander-form-categoryid" name="category_id" type="text" placeholder="category" class="lander-form__input" value="' + inputCategoryId + '"></div><div class="lander-form__group"> <label class="lander-form__label" for="event_id">event_id</label> <input id="lander-form-eventid" name="event_id" type="text" placeholder="event_id" class="lander-form__input" value="' + inputEvent + '"></div><div class="lander-form__group"> <input name="form" type="text" placeholder="form" class="lander-form__input" value="' + inputName + '" disabled></div><div class="form__item form__item_button"> <button class="lander-form__button" type="button">Отправить</button></div></div></form></td><td width="50%" valign="top"><div id="lander-form-container"></div></td></tr></table><div class="sa-button-container"> <button id="lander-form-test-payments-button" style="color: rgb(255, 255, 255); border: medium none; font-size: 18px; font-weight: 500; border-radius: 5px; line-height: 44px; cursor: pointer; margin-top: 35px; padding: 0 15px; display: inline-block; background-color: rgb(174, 222, 244); box-shadow: 0 0 2px rgba(174, 222, 244, 0.8), 0 0 0 1px rgba(0, 0, 0, 0.05) inset;">Закрыть</button></div></div></div></div>';
                    ...
                    $(document).on('click', '#lander-form-test-payments-form button', function() {
                        ...
                        inputEventId.type = 'text';
                        inputEventId.name = 'event_id';
                        inputEventId.value = $('#lander-form-test-payments-form input[name="event_id"]').val();

                        if(callbackSuccess) {
                            var inputCallbackSuccess = document.createElement('input');
                            inputCallbackSuccess.type = 'text';
                            inputCallbackSuccess.name = 'callbackSuccess';
                            inputCallbackSuccess.value = callbackSuccess;
                        }
                        ...
                        if( $('#lander-form-test-payments-form input[name="category"]').val() != 'undefined') {
                            var inputPaymentType = document.createElement('input');
                            inputPaymentType.type = 'text';
                            inputPaymentType.name = 'payment-type';
                            inputPaymentType.value = '1';

                            var inputCategoryName = document.createElement('input');
                            inputCategoryName.type = 'text';
                            inputCategoryName.name = 'category';
                            inputCategoryName.value = $('#lander-form-test-payments-form input[name="category"]').val();
                        }

                        if( $('#lander-form-test-payments-form input[name="category_id"]').val() != 'undefined') {
                            var inputCategoryId = document.createElement('input');
                            inputCategoryId.type = 'text';
                            inputCategoryId.name = 'category_id';
                            inputCategoryId.value = $('#lander-form-test-payments-form input[name="category_id"]').val();;

                            var inputCategory = document.createElement('input');
                            inputCategory.type = 'text';
                            inputCategory.name = 'category';
                            inputCategory.value = $('#lander-form-test-payments-form input[name="category"]').val();
                        }
                        ...
                        if(callbackSuccess) {
                            form.appendChild(inputCallbackSuccess);
                        }

                        if( $('#lander-form-test-payments-form input[name="category"]').val() != 'undefined') {
                            form.appendChild(inputPaymentType);
        ...
        $form_obj.trigger('inited-form.lander');
    };

    /* Автоматическая подстановка маски под номер телефона */
    LANDER.inputMasks = function($context) {
        if (typeof $.fn.inputmask == 'undefined') {
            LANDER.loadJS('https://cdn.synergy.ru/libs/jquery.inputmask/jquery.inputmask-5.0.5.min.js', () => { /* Плагин маски */
                LANDER.loadJS('https://cdn.synergy.ru/libs/jquery.inputmask/jquery.inputmask.binding-5.0.5.min.js', () => {
                    LANDER.loadJS('https://cdn.synergy.ru/libs/inputmask.phone/inputmask.phone.extensions.js', () => {
                        LANDER.loadJS('https://cdn.synergy.ru/libs/inputmask.phone/phone-codes/phone.js', () => {
                            LANDER.loadJS('https://cdn.synergy.ru/libs/inputmask.phone/phone-codes/phone-ru.js', LANDER.inputMasks, true)
                        }, true);
                    }, true); /* Маски телефонов */
                }, true);
            }, true);
            return;
        }
        ...
        if (typeof $context == 'undefined') {
            $context = 'form:not(.nolander)';
        }

        $('[name="phone"]:not([data-inputmasks-inited],[data-inputmask]), [type="tel"]:not([data-inputmasks-inited],[data-inputmask])', $context).each(function() {
            ...
            $(this).attr('autocomplete', 'off').inputmask({
                alias: 'phone',
            ...
            $('body').on('keydown', '[name="phone"]', function(e){
                if (
                    $(this).inputmask("isComplete") && $(this).val().substr(0, $(this).val().length - 1) != '+971-'){
                    return false;
                }
            });
        ...
        $($context).trigger('inited-inputmasks.lander');
    };
    ...
    LANDER.formFields = function(form) {
        ...
        $(form).find('input,select,textarea').not('[type=submit]').each(function() {
            if ($(this).attr('name') && $(this).attr('name') != 'radio' && $(this).attr('type') != 'radio')
            ...
            if ($(this).attr('type') == 'checkbox') {
                fields[$(this).attr('name')] = $(this).prop("checked") ? $(this).val() : '';
            }
        });
    ...
    /* Валидация формы */
    LANDER.formValidation = function($form_obj) {
        if (typeof $.validator == 'undefined') {
            LANDER.loadJS('https://cdn.synergy.ru/libs/jquery.validate/jquery.validate-1.14.0.min.js', LANDER.formValidation, true);
            return;
        }
        ...
        $form_obj.not('#mse2_form, .nolander').each(function() {
            var
                $form = $(this),
            ...
            $form.trigger('inited-formvalidation.lander');
        });
    ...
    LANDER.outlineagency = function() {
        ...
        if (window.location.hostname.indexOf('synergy.ru') != -1) return;
    ...
    LANDER.privacy = function() {
        ...
        if (!$.fn.fancybox) {
            LANDER.loadJS('https://cdn.synergy.ru/libs/fancybox/jquery.fancybox-3.5.7.min.js', initFancyPrivacy , true);
            $('head').append('<link href="https://cdn.synergy.ru/libs/fancybox/jquery.fancybox-3.5.7.min.css" rel="stylesheet">');
        } else {
            initFancyPrivacy();
        }
        ...
        function initCheckboxPrivacy() {
            ...
            if (
                ...
                $('form:not(.nolander)').each(function() {
                    ...
                    var
                        ...
                        offer_text = 'Оплачивая, вы соглашаетесь с условиями публичной оферты'
                        ;
                    ...
                    if (lang == 'en') {
                        ...
                        offer_text = 'By paying, you agree to the terms of the public offer'
                        ;
                    }

                    $form.append('<div class="widget-form-privacy"><div class="widget-form-privacy__content"><input type="checkbox" name="personalDataAgree" checked="" class="widget-form-privacy__checkbox"><label class="widget-form-privacy__label">' + privacy_text + '</label></div><div class="widget-form-privacy__content"><input type="checkbox" name="publicOffer" checked="" class="widget-form-privacy__checkbox"><label class="widget-form-privacy__label">' + offer_text + '</label></div></div>');
                });
            }

            /* Проверяем чекбокс, дизаблим/раздизабливаем кнопку */
            $(document).on('change init.checkbox', ':checkbox', function() {
                var
                $form = $(this).closest('form'),
                $privacy = $form.find('.widget-form-privacy'),
                $submit = $(':submit', $form),
                checkboxes_nonchecked = $(':checkbox', $form).not('.nocheck').not(':checked').length
                ;

                if (checkboxes_nonchecked === 0) {
            ...
            $(':checkbox').trigger('init.checkbox');
        }
    };


    /* Подгрузка скрипта и выполнение функции после его загрузки  */
    LANDER.loadJS = function(js_file, js_function, js_async) {
    ...
    LANDER.initWidgetVacancy = function() {
        ...
        var enabled = lands.indexOf(location.host + location.pathname) >= 0 ||
                location.host == "synergy.ru" && location.pathname.slice(1, 3) == "lp" &&
                location.hostname.indexOf('synergy.ru/lp/management') != -1;
    ...
    /* Функция для проброса в лендер значения из getVisitorId() Матомо, через поле matomo_uid, добавляемого в форму */
    LANDER.initMatomo = function(form) {
        if ( typeof window._paq != 'object' || !form ) return;
        ...
        window._paq.push([ function(){ visitor_id = this.getVisitorId(); } ]);

        input.type = 'hidden';
        input.name = 'matomo_uid';
        input.value = visitor_id;
